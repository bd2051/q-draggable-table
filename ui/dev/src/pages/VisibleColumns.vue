...
export default {
  setup () {
    ...
    return {
      pagination: ref(null),
      visibleColumns: ref([ 'name', 'calories', 'fat', 'carbs', 'protein', 'sodium', 'calcium', 'iron' ]),
      columns,
      rows
    }
  },
    ...
    visibleColumns: {
      handler(val) {
        ...
        this.columns.sort((a, b) => {
          const maxSortIndex = this.columns.length + 1
          return (visibleColumnsMap[a.name] || maxSortIndex) - (visibleColumnsMap[b.name] || maxSortIndex)
        })
      },
      immediate: true
    }
